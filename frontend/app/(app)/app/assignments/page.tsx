...
export default function AssignmentsPage() {
  ...
  const { isStudent, isTeacher } = useAuthStore();
  ...
  const courseIds = useMemo(() => {
    if (isStudent() && enrollmentsData?.enrollments) {
      return enrollmentsData.enrollments
        .filter((e) => e.status === "active")
        .map((e) => e.course_id);
    }
    return coursesData?.courses?.map((c) => c.id) || [];
  }, [isStudent, enrollmentsData, coursesData]);
  ...
  const { data: mySubmissionsData } = useQuery({
    ...
    queryFn: () => submissionService.getMySubmissions({ limit: 200 }),
    enabled: isStudent(),
  });
  ...
  const courses = useMemo(() => {
    ...
    return (
      coursesData?.courses?.map((c) => ({ id: c.id, title: c.title })) || []
    );
  }, [isStudent, enrollmentsData, coursesData]);
  ...
  return (
    ...
    >
      <PageHeader
        ...
        description="View and track all your assignments across enrolled courses"
      />
