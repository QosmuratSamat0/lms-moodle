...
export default function DeadlinesPage() {
  ...
  const { user, isStudent, isTeacher } = useAuthStore();
  ...
  // Get course IDs from enrollments (students) or course list (teachers)
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["my-deadline-assignments", courseIds],
    queryFn: async () => {
      if (courseIds.length === 0) return { assignments: [], total: 0 };
