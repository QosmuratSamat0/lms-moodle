...
export const assignmentService = {
  list: async (
    ...
    // Backend has no generic /assignments list.
    // If course_id provided, use /assignments/course/:courseId
    // Otherwise return empty
    if (params?.course_id) {
      const assignments = await api.get<any>(
        `/assignments/course/${params.course_id}`,
        params as QueryParams,
      );
      const list = Array.isArray(assignments)
        ? assignments
        : assignments?.assignments || [];
      return { assignments: list, total: list.length };
    }
    // No course_id - return empty since backend has no list-all endpoint
...
export default (useMock ? assignmentServiceMock : assignmentService) as typeof assignmentService;
